...
import { secondsSince } from "@workspace/evoverses/utils/numbers";
...
const AssetBreeding = ({ tokenId }: { tokenId: string }) => {
  ...
  const attributes = [
    ...
    { name: "Last Breed Time", value: Math.floor(secondsSince(metadata.lastBreedTime) / 86400) + " Days" },
    {
      name: "Next Breed Status", value: secondsSince(metadata.lastBreedTime) > 604_800
        ? "Ready"
        : Math.floor((
        secondsSince(metadata.lastBreedTime) + 604_800
      ) / 86_400) + " Days",
    },
