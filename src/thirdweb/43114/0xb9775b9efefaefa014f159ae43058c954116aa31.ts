...
type GetAllDataParams = {
  address: Address
}
export const getAllLpData = async (options: BaseTransactionOptions<GetAllDataParams>) => {
  const [ totalSupply_, balance, token0_, token1_, reserves ] = await Promise.all([
    totalSupply(options),
    balanceOf({ ...options, arg_0: options.address }),
    token0(options),
    token1(options),
    getReserves(options),
  ]);
